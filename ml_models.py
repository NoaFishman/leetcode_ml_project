...
def save_best_model_info(best_model_name, model_metrics, threshold):
    ...
    joblib.dump(best_model_info, 'models/best_model_related_topics_info.pkl')
...
def related_topics_prediction():
    ...
    joblib.dump(vectorizer, 'models/related_topics_vectorizer.pkl')

    mlb = MultiLabelBinarizer(classes=all_possible_topics)
    y = mlb.fit_transform(df['related_topics'])
    joblib.dump(mlb, 'models/related_topics_label_binarizer.pkl')
    ...
    joblib.dump(trained_best_model, "models/best_related_topics_model.pkl")
...
def train_and_evaluate(the_data, top_in_tfidf, target_column, metrics_dict, model_type="linear_regression"):
    ...
    if target_column == "likes" and model_type == "random_forest":
        ...
        joblib.dump((model, feature_names), "models/likes_random_forest_regression_model.pkl")
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f"plots/_{target_column} Prediction Performance - {model_type.replace('_', ' ').title()}.jpg")
...
def evaluate_model(y_test, y_pred, model_name, target_column, metrics_dict):
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f"plots/_{target_column} Prediction Performance - {model_name}.jpg")
...
def plot_comparison_histogram(metrics_dict, target_column):
    ...
    ax.legend()
    # plt.show()
    plt.savefig(f"plots/_Comparison of Models for {target_column.capitalize()}.jpg")
...
def xgboost_for_dislikes(the_data, top_in_tfidf, metrics_dict):
    ...
    feature_names = list(X_train.columns)
    joblib.dump((model, feature_names), "models/dislikes_XGB_regression_model.pkl")
...
def accepted_submissions_regression(df):
    ...
    joblib.dump((model, feature_names), "models/accepted_submissions_regression_model.pkl")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"plots/_Actual vs Predicted Accepted Submissions.jpg")
...
def difficulty_classification(df, top_in_tfidf, related_topics, nn_result):
    ...
    joblib.dump(trained_best_model, "models/level_classifier_model.pkl")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"plots/_Classifier Accuracy Comparison.jpg")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"plots/_{name} (Level {level}).jpg")
...
def difficulty_classification_nn(df, top_in_tfidf, related_topics, seed=42):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"plots/_Accuracy Over Time.jpg")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"plots/_Actual vs predicted difficulty level.jpg")
